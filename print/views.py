    ...
    if event == "PrinterStateChanged":
        ...
        try:
            timestamp = datetime.fromtimestamp(
                json.loads(m)["_timestamp"], tz=pytz.timezone("Europe/Berlin")
            )
        ...
        try:
            if p_exists:
                ...
                if tool_bed == "bed":
                    try:
                        BedTemperatureHistory.objects.create(
                            PrintJob_id=PrintJob.objects.get(
                                Machine_id=printer_id, State="PRINTING"
                            ).id,
                            Target=target,
                            Actual=actual,
                            TimeStamp=timestamp,
                ...
                # If Temperature is tool info
                    ...
                    try:
                        ToolTemperatureHistory.objects.create(
                            PrintJob_id=PrintJob.objects.get(
                                Machine_id=printer_id, State="PRINTING"
                            ).id,
                            Target=target,
        ...
        try:
            ...
            try:
                PrintMediaFile.objects.get(
                    PrintJob_id=printjob_id, Description="timelapse"
                )
            except:
